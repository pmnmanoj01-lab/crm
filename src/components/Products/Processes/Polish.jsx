...
const Polish = () => {
  ...
  const ROLE_LIST = [ProductionRoles.Polish];
    ...
    const fetchProduct = async () => {
      try {
        const res = await fetch(
          `${backendRoute}${routes.getProductPolish}${productId}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = product
        ? `${backendRoute}${routes.updateProductPolish}${productId}`
        : `${backendRoute}${routes.createProductPolish}`;
  ...
  return (
    <div className="max-w-full mx-auto bg-white p-6 rounded-xl mt-5">
      <h2 className="text-2xl font-bold mb-4 text-gray-700">
        {productId ? "Edit Polish" : "Polish Form"}
      </h2>
