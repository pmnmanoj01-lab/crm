...
const Filing = ({ material, onProcessUpdated }) => {
    ...
    const fetchProduct = async () => {
      try {
        const res = await fetch(
          `${backendRoute}${routes.getProductFiling}${productId}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = product
        ? `${backendRoute}${routes.updateProductFiling}${productId}`
        : `${backendRoute}${routes.createProductFiling}`;
  ...
  return (
    <div className="max-w-full mx-auto bg-white p-6 rounded-xl mt-5">
      <h2 className="text-2xl font-bold mb-4 text-gray-700">
        {productId ? "Edit Filing" : "Filing Form"}
      </h2>
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          ...
          {productId && (
            <div>
              ...
              <input
                type="text"
                className="w-full border rounded-lg px-3 py-2 bg-gray-100 border-gray-300 focus:outline-none"
        ...
        {/* -------------------- EXTRA MATERIAL -------------------- */}
        <div className="mt-6 p-4 border rounded-lg bg-gray-50 border-gray-300 ">
